...
exports.sectionFile = async (req, res) => {
  if (req.file == undefined) {
    return res.status(400).send({ message: "Please upload a CSV file!" });
  }
  try {
    let fileContent = await fs.readFile(
      "filehandling/files/" + req.file.filename
    );
    ...
    for (let rowIndex = 0; rowIndex < records.length; rowIndex++) {
      ...
      for (let varIndex = 0; varIndex < rowBldgArray.length; varIndex++) {
        ...
        roomId = await findRoomId(
          rowBldgArray[varIndex],
          rowRoomArray[varIndex]
        );

        //SectionTimes
        await findSectionTimeId(
          rowStartTimeArray[varIndex],
          rowEndTimeArray[varIndex],
          rowStartDateArray[varIndex],
          rowEndDateArray[varIndex],
          rowSundayArray[varIndex],
          rowMondayArray[varIndex],
          rowTuesdayArray[varIndex],
          rowWednesdayArray[varIndex],
          rowThursdayArray[varIndex],
          rowFridayArray[varIndex],
          rowSaturdayArray[varIndex],
          sectionId,
          roomId
        );
      }
    ...
    fs.unlink("filehandling/files/" + req.file.filename);
    ...
    res.status(500).send({
      message:
        "There was a problem uploading the file: " + req.file.originalname,
    });
  }
};

exports.courseFile = async (req, res) => {
  if (req.file == undefined) {
    return res.status(400).send({ message: "Please upload a CSV file!" });
  }
...
async function findFacultyId(input) {
  ...
  var tempFaculty = facultyArray.find((fac) => fac.name === name);
...
async function findSemesterId(inputSemesterCode) {
  var tempSemester = semesterArray.find(
    (semester) => semester.code === inputSemesterCode
  );
  ...
  if (tempSemester === null || tempSemester === undefined) {
    ...
    await Semesters.create(semester)
      .then((data) => {
...
async function findSectionId(inputSectionNum, inputCourseId, inputSemesterId) {
  var tempSection = sectionArray.find(
    (section) =>
      section.number === inputSectionNum && section.courseId === inputCourseId
  );
...
async function findFacultySectionId(inputFacultyId, inputSectionId) {
  var tempFacultySection = facultySectionArray.find(
    (fs) => fs.sectionID === inputSectionId && fs.facultyID === inputFacultyId
  );
...
async function findSectionTimeId(
  inputStartTime,
  inputEndTime,
  inputStartDate,
  inputEndDate,
  ...
  var tempSectionTime = sectionTimeArray.find(
    (st) =>
      (st.startTime === inputStartTime ||
        (st.startTime === "00:00:00" && inputStartTime === "")) &&
      (st.endTime === inputEndTime ||
        (st.endTime === "00:00:00" && inputEndTime === "")) &&
      st.startDate === inputStartDate &&
      st.endDate === inputEndDate &&
      st.sectionId === inputSectionId &&
      st.roomId === inputRoomId
  );
...
function validateSectionHeaders(records) {
  ...
  if (records["Bldg"] === undefined) {
    headerErrorArray.push("Bldg");
  }
  if (records["Room"] === undefined) {
    headerErrorArray.push("Room");
  }
  if (records["Start Time 24hr"] === undefined) {
    headerErrorArray.push("Start Time 24hr");
  }
  if (records["End Time 24hr"] === undefined) {
    headerErrorArray.push("End Time 24hr");
  }
  if (records["Meeting Start Date"] === undefined) {
    headerErrorArray.push("Meeting Start Date");
  }
  if (records["Meeting End Date"] === undefined) {
    headerErrorArray.push("Meeting End Date");
  }
  if (records["Sun"] === undefined) {
    headerErrorArray.push("Sun");
  }
  if (records["Mon"] === undefined) {
    headerErrorArray.push("Mon");
  }
  if (records["Tue"] === undefined) {
    headerErrorArray.push("Tue");
  }
  if (records["Wed"] === undefined) {
    headerErrorArray.push("Wed");
  }
  if (records["Thu"] === undefined) {
    headerErrorArray.push("Thu");
  }
  if (records["Fri"] === undefined) {
    headerErrorArray.push("Fri");
  }
  if (records["Sat"] === undefined) {
    headerErrorArray.push("Sat");
  }
  if (records["Faculty Name (LFM)"] === undefined) {
    headerErrorArray.push("Faculty Name (LFM)");
  }
  if (records["Faculty Name 2 (LFM)"] === undefined) {
    headerErrorArray.push("Faculty Name 2 (LFM)");
  }
  if (records["Subject"] === undefined) {
    headerErrorArray.push("Subject");
  }
  if (records["Course #"] === undefined) {
    headerErrorArray.push("Course #");
  }
  if (records["Section Title"] === undefined) {
    headerErrorArray.push("Section Title");
  }
  if (records["Term"] === undefined) {
    headerErrorArray.push("Term");
  }
  if (records["Section #"] === undefined) {
    headerErrorArray.push("Section #");
  }
...
function validateCourseHeaders(records) {
  ...
  if (records["Short Title"] === undefined) {
    headerErrorArray.push("Short Title");
  }
